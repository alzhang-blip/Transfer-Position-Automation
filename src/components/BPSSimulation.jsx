...
export default function BPSSimulation({ task }) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold text-slate-200">BPS Entry Simulation</h3>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="border border-red-800/50 rounded-lg overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            ...
            <Row label="Symbol" value={task.symbol} mono />
            <Row label="Quantity" value={task.quantity.toLocaleString()} mono />
      ...
      <div className="p-3 rounded-lg bg-[#0d1117] border border-[#2d333b]">
        <p className="text-[10px] text-slate-500 leading-relaxed">
          Internal offset account <span className="font-mono font-medium text-slate-400">{offsetAccount}</span> is used to balance the DW/DC pair.
          Both entries must be submitted together as a matched set in BPS.
        </p>
      </div>
    </div>
  );
